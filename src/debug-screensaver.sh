...
if [ -z "${DBUS_SESSION_BUS_ADDRESS}" ]; then
    pid=`pgrep -u $USER "cafe-session|x-session-manager"`
    if [ "x$pid" != "x" ]; then
...
cafe-screensaver-command --exit

# run the daemon in the debugger
#gdb --args cafe-screensaver --no-daemon --debug --sync

# or if that isn't helpful just get the debug output
#cafe-screensaver --no-daemon --debug > /tmp/gs-debug-log.txt 2>&1

# or just run it with debugging on
cafe-screensaver --no-daemon --debug
